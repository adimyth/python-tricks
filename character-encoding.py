"""
Text files are collection of bytes. Each file can be stored & read from using different encodings.
...
with open("text-files/sample.txt", encoding="ISO-8859-1") as f:
    s = f.read()
print(f"Type: {type(s)}\nContent: {s}")
...
with open("text-files/sample.txt", "rb") as f:
    raw_data = f.read()
